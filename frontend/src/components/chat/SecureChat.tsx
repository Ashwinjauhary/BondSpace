...
export default function SecureChat() {
    ...
    useEffect(() => {
        ...
        const handleMsg = async (msg: Message) => {
            ...
            try {
                ...
                if (encryptionKey) {
                    const dec = await decryptMessage(msg.message, encryptionKey);
                    if (dec !== "[Decryption Failed - Invalid Key]") {
                        plaintext = dec;
                    }
                }
    ...
    const handleSendSticker = async (mediaUrl: string) => {
        if (!socket) {
            alert("No socket connection. Please wait.");
            return;
        }
        setShowStickers(false);
        try {
            console.log("Preparing to send sticker:", mediaUrl);
